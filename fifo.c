...
main (int argc, char* argv[]){
	if (argc < 4){
		printf("Introduzca los argumentos de la función, por ejemplo: ./fifo nombre_fichero.txt tamTLB tamPag.\n");
		exit(-1);
	}
	if (argc > 4){
		printf("Demasiados argumentos para la función. Introduzca por ejemplo: ./fifo nombre_fichero.txt tamTLB tamPag.\n");
		exit(-1);
	}
